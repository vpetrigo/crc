...
TEST(CRC8Test, BasicTest)
{
    ...
    uint8_t expect = 0xa2;
    EXPECT_EQ(crc8_msb(reinterpret_cast<const unsigned char *>(inData), 9),
              expect);

    const char *data = "Hello World!!!";
    expect = 0x68;
    EXPECT_EQ(
        crc8_msb(reinterpret_cast<const unsigned char *>(data), strlen(data)),
        expect);

    expect = 0x2c;
    EXPECT_EQ(
        crc8_lsb(reinterpret_cast<const unsigned char *>(data), strlen(data)),
        expect);
...
TEST(CRC8Test, UpdateTest)
{
    ...
    const std::array test_cases_lsb{
        ...
        TestCase{
            to_array("\xb0\x00\x00\x00\x00\x00\x00", 7),
            0x1a,
        },
    };
    ...
    for (const auto &[in_data, expect] : test_cases_msb) {
        ...
        crc =
            crc8_msb_update(crc, in_data.data() + total / 2, total - total / 2);
    ...
    for (const auto &[in_data, expect] : test_cases_lsb) {
        ...
        crc =
            crc8_lsb_update(crc, in_data.data() + total / 2, total - total / 2);
        EXPECT_EQ(crc, expect);
    }
}
...
{
    constexpr std::array<const unsigned char, 9> inData = {
        '1', '2', '3', '4', '5', '6', '7', '8', '9'};
    uint32_t expect = 0xCBF43926;
...
TEST(CRC64Test, BasicTest)
{
    ...
    EXPECT_EQ(crc64(reinterpret_cast<const unsigned char *>(inData), 9),
              expect);
}
